...
@property (nonatomic, copy) JSObservingBlock block;

@end

@implementation PGObservationInfo

- (instancetype)initWithObserver:(NSObject *)observer Key:(NSString *)key block:(JSObservingBlock)block {
    self = [super init];
    if (self) {
        _observer = observer;
        _key = key;
...
@implementation NSObject (JS_KVO)
